...
/**
  ...
  frequency.
*/
uint8_t gzll_chm_get_current_rx_channel(void);
...
/*
  Dynamic parameters.
*/
static uint16_t gzll_dyn_params[GZLL_DYN_PARAM_SIZE];

/*
  Channel subset array.
*/
static uint8_t gzll_channel_tab[GZLL_MAX_CHANNEL_TAB_SIZE] = GZLL_DEFAULT_CHANNEL_TAB;

/*
  Pipe 0 address shadow register.
*/
static uint8_t gzll_p0_adr[GZLL_ADDRESS_WIDTH] = GZLL_DEFAULT_ADDRESS_PIPE0;

#define GZLL_BIT(_var, _bitnum) bool volatile _var
...
void gzll_timer_isr_function(void)
{
  ...
  //flag = gzll_interupts_save();
  dbg_irq_flag = hal_nrf_get_irq_flags();
...
static uint8_t gzll_chm_rx_channel_index = 0;
static uint8_t gzll_chm_hold_rx_channel_index = 0;
static uint16_t gzll_chm_rx_channel_hold = 0;
static uint16_t gzll_chm_rx_mode1_sequence = 0;
...
uint8_t gzll_chm_get_current_rx_channel()
{
  if(gzll_chm_rx_channel_hold > 0)
  {
    return gzll_chm_hold_rx_channel_index;
